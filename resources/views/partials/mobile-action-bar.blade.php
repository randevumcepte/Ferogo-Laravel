{{-- Mobil sticky action bar: Ara + WhatsApp. Layout veya panel sayfasının body içine include edilir.
     Embed modunda (iframe) gizlenir. Kaydırınca alttan kayarak gelir. --}}
...
(function() {
    ...
    function setShown(v) {
        if (v === shown) return;
        shown = v;
        if (v) bar.classList.remove('translate-y-full', 'opacity-0', 'pointer-events-none');
        else   bar.classList.add('translate-y-full', 'opacity-0', 'pointer-events-none');
    }
    window.addEventListener('scroll', () => setShown(window.scrollY > SHOW_AFTER), { passive: true });
