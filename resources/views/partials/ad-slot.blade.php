{{--
    ...
    - Reklamda "Tam görsel" açıksa: görsel tüm alanı kaplar (kırpılmaz, yazı/buton yok).
    - Değilse: solda görsel + sağda başlık/açıklama/buton.
    - Aktif reklam yoksa: alanın önerilen ölçüsünü gösteren boş yer tutucu.
--}}
@php
    ...
    $slotDims = \App\Modules\Marketing\Models\Advertisement::dimensionsFor($placement);
...
@if ($ad && $ad->image_only && $ad->image_src)
    ...
    <a href="{{ $ad->link_url ? route('ad.click', $ad) : '#' }}"
       ...
       class="ad-slot group relative block overflow-hidden rounded-3xl border-2 border-brand/40 hover:border-brand/70 shadow-[0_14px_50px_-14px_rgba(240,192,64,0.5)] hover:-translate-y-0.5 transition duration-300 {{ $slotClass }}">
        ...
        <img src="{{ $ad->image_src }}" alt="{{ $ad->title }}" class="block w-full h-auto" loading="lazy">
    </a>
@elseif ($ad)
            ...
            <div class="shrink-0 relative {{ $compact ? 'w-full' : 'sm:w-64 lg:w-72' }}">
                ...
                @else
                    ...
                    <div class="border-2 border-dashed border-brand/40 bg-brand/[0.06] flex flex-col items-center justify-center text-center gap-1.5 text-brand/75 rounded-2xl {{ $compact ? 'h-28 m-3' : 'h-40 sm:h-auto sm:absolute sm:inset-3 m-4 sm:m-0' }}">
                        ...
                        <div class="text-[10px] text-brand/50">{{ $slotDims }}</div>
                    </div>
                @endif
            </div>
...
@else
    ...
    <div class="ad-slot ad-slot--empty relative rounded-3xl border-2 border-dashed border-brand/60 bg-gradient-to-br from-brand/[0.12] to-transparent text-center shadow-[0_12px_45px_-12px_rgba(240,192,64,0.3)] p-8 {{ $slotClass }}">
        ...
        <div class="text-[11px] text-zinc-500 mt-1">{{ $slotSeg }}</div>
