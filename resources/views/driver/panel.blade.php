...
<body class="bg-black text-white min-h-screen pb-20 md:pb-0">
            ...
            <div class="rounded-3xl border border-emerald-500/30 bg-emerald-500/[0.05] overflow-hidden">
                <div class="p-5 border-b border-white/5 flex items-center justify-between gap-3 flex-wrap">
                    ...
                    <div class="flex items-center gap-2 flex-wrap">
                        ...
                        {{-- Eşleşme kodu ile başlat — vardıktan sonra görünür, kod modalını açar --}}
    ...
    (function() {
        ...
        function renderBoardingStartState(a) {
            ...
            // Henüz vardı değil → butonlar gizli
            if (!arrived) {
            ...
            // Yolculuk başladı → sadece "Tamamlandı"
            if (started) {
                startBtn.classList.add('hidden');
            ...
            // Vardı, başlamadı → "Kodu Gir & Başlat" + kod modalı bir kez otomatik açılır
            startBtn.classList.remove('hidden');
            completeBtn.classList.add('hidden');
            if (codeModalShownFor !== a.public_id) {
                codeModalShownFor = a.public_id;
                openCodeModal();
            }
        }

        let codeModalShownFor = null;

        function openCodeModal() {
            const modal = $('match-code-modal');
            if (!modal) return;
            modal.classList.remove('hidden');
            modal.classList.add('flex');
            const err = $('match-code-error'); if (err) err.textContent = '';
            const inp = $('match-code-input'); if (inp) { inp.value = ''; setTimeout(() => inp.focus(), 50); }
        }
        ...
        // ===== Eşleşme kodu ile yolculuğu başlatma =====

        // "Kodu Gir & Başlat" butonu → kod modalını açar
        $('active-start-ride').addEventListener('click', () => openCodeModal());

        // Kod modalı: kapat
        document.addEventListener('click', (e) => {
            if (e.target.closest('#match-code-cancel')) closeCodeModal();
        });

        // Kod modalı: doğrula & başlat
        async function submitMatchCode() {
            const inp = $('match-code-input');
            const err = $('match-code-error');
            const btn = $('match-code-submit');
            const code = (inp.value || '').trim();
            if (code.length !== 4) { err.textContent = 'Kod 4 haneli olmalı.'; return; }
            ...
            try {
                ...
                if (data.ok) {
                    ...
                    err.textContent = data.message || 'Kod doğrulanamadı.';
                    inp.value = ''; inp.focus();
                }
            } catch (_) {
        ...
        document.addEventListener('click', (e) => {
            if (e.target.closest('#match-code-submit')) submitMatchCode();
        });
        document.addEventListener('keydown', (e) => {
            if (e.key === 'Enter' && document.activeElement && document.activeElement.id === 'match-code-input') {
                e.preventDefault(); submitMatchCode();
            }
        });
    ...
    <div id="match-code-modal"
         class="fixed inset-0 z-[120] hidden items-center justify-center bg-black/85 backdrop-blur-md px-4 py-6"
         role="dialog" aria-modal="true">
        <div class="w-full max-w-md rounded-3xl bg-zinc-900 border border-brand/40 shadow-2xl shadow-brand/20">
            <div class="px-6 pt-6 pb-3">
                <div class="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-brand/15 border border-brand/40 text-brand text-[11px] uppercase tracking-[0.2em] font-bold mb-3">
                    <span class="text-base">🔑</span> Eşleşme Kodu
                </div>
                <h2 class="text-2xl font-bold text-white leading-tight">
                    Yolculuğu başlat
                </h2>
                <p class="text-sm text-zinc-400 mt-2 leading-relaxed">
                    Yolcunun uygulamasında görünen <strong class="text-brand">4 haneli kodu</strong> iste ve buraya gir.
                    Doğruysa yolculuk başlar.
                </p>
            </div>

            <div class="px-6 py-5 flex flex-col gap-3">
                <input id="match-code-input" type="tel" inputmode="numeric" maxlength="4" autocomplete="off"
                       placeholder="••••"
                       class="w-full text-center tracking-[0.6em] text-3xl font-black text-white bg-black/40 border border-white/10 rounded-xl py-4 focus:outline-none focus:border-brand">
                <p id="match-code-error" class="text-center text-sm text-red-400 min-h-[20px]"></p>
                <button type="button" id="match-code-submit"
                    ...
                    <span>▶</span> Doğrula ve başlat
                </button>
                <button type="button" id="match-code-cancel"
